...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function updateStepClasses(prevIndex: number, nextIndex: number, direction: 'left' | 'right') {
    ...
    function goToNextStep() {
        if (currentStep >= steps.length - 1) {
            ...
            window.location.href = "/src/pages/location.html";
            return;
        }
    ...
    function handleSwipeGesture() {
        ...
        if (!isLastStep && touchStartX - touchEndX > threshold) {
            // سوایپ به چپ فقط تا قبل از آخرین اسلاید مجازه
            goToNextStep();
        } else if (currentStep > 0 && touchEndX - touchStartX > threshold) {
            // سوایپ به راست همیشه فعال بمونه (برای برگشت)
            goToPrevStep();
        }
    }
    ...
    nextBtn.addEventListener("click", () => {
        // فقط در آخرین اسلاید اجازه ورود
        if (currentStep === steps.length - 1) {
            window.location.href = "/src/pages/location.html";
        } else {
            goToNextStep();
